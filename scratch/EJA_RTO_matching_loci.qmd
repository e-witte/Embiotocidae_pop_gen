...
for(i in 1:nrow(RTO_raw_loci)){
  if(check_match(locus = RTO_raw_loci$BAR[i], thresh = dist, table = EJA_raw_loci$BAR))
...
for(i in 1:nrow(RTO_raw_loci_select)){
  if(check_match(locus = RTO_raw_loci_select$BAR[i], thresh = dist, table = EJA_raw_loci$BAR))
...
for(i in 1:nrow(EJA_raw_loci)){
  if(check_match(locus = EJA_raw_loci$BAR[i], thresh = dist, table = RTO_raw_loci$BAR))
...
for(i in 1:nrow(EJA_raw_loci_select)){
  if(check_match(locus = EJA_raw_loci_select$BAR[i], thresh = dist, table = RTO_raw_loci$BAR))
...
RTO_EJA <- data.frame(iteration=1:iter, matches= NA) #create unfilled data frame
...
for(z in 1:length(RTO_sample)) {
  if(check_match(locus = RTO_sample[z], thresh = dist, table = EJA_sample))
    match_count <- match_count + 1
}
RTO_EJA$matches[i] <- match_count
} # Runs iterations of matches-- populates RTO_EJA$matches

nrow(RTO_EJA)
mean_RTO_EJA<- mean(RTO_EJA$matches)

##Plot and determine significance
###Observed matches
obs_loci_RTO_EJA <- data.frame(matrix(ncol = 6, nrow = 0))
colnames(obs_loci_RTO_EJA) <- colnames(RTO_raw_loci)

for(i in 1:nrow(RTO_screen_loci_select)){
if(check_match(locus = RTO_screen_loci_select$BAR[i], thresh = dist, table = EJA_screen_loci_select$BAR))
  obs_loci_RTO_EJA <- rbind(obs_loci_RTO_EJA, RTO_screen_loci_select[i,])
}

obs<-nrow(obs_loci_RTO_EJA)
obs

###Probability distribution function
prob_df<- ggplot(RTO_EJA, aes(x=matches)) +geom_histogram(binwidth=1) + geom_vline(aes(xintercept= mean_RTO_EJA), linetype="dashed") + geom_vline(aes(xintercept = obs), colour="green") + (labs(title= "Probability distribution between E. jacksoni and R. toxotes"))

ggsave(filename = "figs/RTO_EJA_prob_df.png", plot = prob_df)

###Cumulative distribution function
cdf<- data.frame(x = 1:(max(RTO_EJA$matches)+1), y = NA)
cdf$y<- cumsum(table(RTO_EJA$matches)) / nrow(RTO_EJA)
qtile<- cdf[obs,2]
...
cdf_plot<- ggplot(cdf, aes(x,y)) + 
  ...
  ggtitle("CDF of matches between R. toxotes and E. jacksoni \n p =", p)+
...
ggsave("figs/RTO_EJA_cum_df.png", cdf_plot)
