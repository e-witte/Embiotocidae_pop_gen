...
for(i in 1:nrow(RTO_raw_loci)){
  if(check_match(locus = RTO_raw_loci$BAR[i], thresh = dist, table = PVA_raw_loci$BAR))
...
for(i in 1:nrow(RTO_raw_loci_select)){
  if(check_match(locus = RTO_raw_loci_select$BAR[i], thresh = dist, table = PVA_raw_loci$BAR))
...
for(i in 1:nrow(PVA_raw_loci)){
  if(check_match(locus = PVA_raw_loci$BAR[i], thresh = dist, table = RTO_raw_loci$BAR))
...
for(i in 1:nrow(PVA_raw_loci_select)){
  if(check_match(locus = PVA_raw_loci_select$BAR[i], thresh = dist, table = RTO_raw_loci$BAR))
...
RTO_PVA <- data.frame(iteration=1:iter, matches= NA) #create unfilled data frame
...
for(z in 1:length(RTO_sample)) {
  if(check_match(locus = RTO_sample[z], thresh = dist, table = PVA_sample))
    match_count <- match_count + 1
}
RTO_PVA$matches[i] <- match_count
} # Runs iterations of matches-- populates RTO_PVA$matches

nrow(RTO_PVA)
mean_RTO_PVA<- mean(RTO_PVA$matches)
...
prob_df<- ggplot(RTO_PVA, aes(x=matches)) +geom_histogram(binwidth=1) + geom_vline(aes(xintercept= mean_RTO_PVA), linetype="dashed") + geom_vline(aes(xintercept = obs), colour="green") + (labs(title= "Probability distribution between P. vacca and R. toxotes"))

ggsave(filename = "figs/RTO_PVA_prob_df.png", plot = prob_df)

###Cumulative distribution function
cdf<- data.frame(x = 1:(max(RTO_PVA$matches)+1), y = NA)
cdf$y<- cumsum(table(RTO_PVA$matches)) / nrow(RTO_PVA)
qtile<- cdf[obs,2]
...
ggsave("figs/RTO_PVA_cum_df.png", cdf_plot)
