...
###Identify outliers using Average + 3sd
EJA_ave_phi <- mean(EJA_raw_loci$phi_st, na.rm = TRUE)
EJA_sd_phi <- sd(EJA_raw_loci$phi_st, na.rm = TRUE)
EJA_thresh_phi <- EJA_ave_phi + 3 * EJA_sd_phi
EJA_raw_loci_select <- EJA_raw_loci %>% filter(EJA_raw_loci$phi_st > EJA_thresh_phi, na.rm = TRUE)
view(EJA_raw_loci_select)

##Screen data
dist <- 1 #set threshold for matches
PVA_raw_loci <- na.omit(PVA_raw_loci)
EJA_raw_loci <- na.omit(EJA_raw_loci)
...
for(i in 1:nrow(EJA_raw_loci)){
  if(check_match(locus = EJA_raw_loci$BAR[i], thresh = dist, table = PVA_raw_loci$BAR))
    EJA_screen_loci <- rbind(EJA_screen_loci, EJA_raw_loci[i,])}
    
EJA_ret <- round((nrow(EJA_screen_loci)/nrow(EJA_raw_loci)), 4)
...
EJA_screen_loci_select <- data.frame(matrix(ncol = 6, nrow = 0))
colnames(EJA_screen_loci_select) <- colnames(EJA_raw_loci)

for(i in 1:nrow(EJA_raw_loci_select)){
  if(check_match(locus = EJA_raw_loci_select$BAR[i], thresh = dist, table = PVA_raw_loci$BAR))
    EJA_screen_loci_select <- rbind(EJA_screen_loci_select, EJA_raw_loci_select[i,])}
    
EJA_ret_select <- round((nrow(EJA_screen_loci_select)/nrow(EJA_raw_loci_select)), 4)
...
for(i in 1:nrow(PVA_raw_loci)){
  if(check_match(locus = PVA_raw_loci$BAR[i], thresh = dist, table = EJA_raw_loci$BAR))
...
for(i in 1:nrow(PVA_raw_loci_select)){
  if(check_match(locus = PVA_raw_loci_select$BAR[i], thresh = dist, table = EJA_raw_loci$BAR))
...
EJA_PVA <- data.frame(iteration=1:iter, matches= NA) #create unfilled data frame
n_select_EJA<- nrow(EJA_screen_loci_select)
n_select_PVA<- nrow(PVA_screen_loci_select)

for(i in 1:iter){
EJA_sample <- sample(EJA_screen_loci$BAR, n_select_EJA, replace = FALSE)
PVA_sample <- sample(PVA_screen_loci$BAR, n_select_PVA, replace = FALSE)
match_count <- 0
for(z in 1:length(EJA_sample)) {
  if(check_match(locus = EJA_sample[z], thresh = dist, table = PVA_sample))
    match_count <- match_count + 1
}
EJA_PVA$matches[i] <- match_count
} # Runs iterations of matches-- populates EJA_PVA$matches

nrow(EJA_PVA)
mean_EJA_PVA<- mean(EJA_PVA$matches)

##Plot and determine significance
###Observed matches
obs_loci_EJA_PVA <- data.frame(matrix(ncol = 6, nrow = 0))
colnames(obs_loci_EJA_PVA) <- colnames(EJA_raw_loci)

for(i in 1:nrow(EJA_screen_loci_select)){
if(check_match(locus = EJA_screen_loci_select$BAR[i], thresh = dist, table = PVA_screen_loci_select$BAR))
  obs_loci_EJA_PVA <- rbind(obs_loci_EJA_PVA, EJA_screen_loci_select[i,])
}

obs<-nrow(obs_loci_EJA_PVA)
obs

###Probability distribution function
prob_df<- ggplot(EJA_PVA, aes(x=matches)) +geom_histogram(binwidth=1) + geom_vline(aes(xintercept= mean_EJA_PVA), linetype="dashed") + geom_vline(aes(xintercept = obs), colour="green") + (labs(title= "Probability distribution between P. vacca and E. jacksoni"))

ggsave(filename = "figs/EJA_PVA_prob_df.png", plot = prob_df)

###Cumulative distribution function
cdf<- data.frame(x = 1:(max(EJA_PVA$matches)+1), y = NA)
cdf$y<- cumsum(table(EJA_PVA$matches)) / nrow(EJA_PVA)
qtile<- cdf[obs,2]
...
cdf_plot<- ggplot(cdf, aes(x,y)) + 
  ...
  ggtitle("CDF of matches between E. jacksoni and P. vacca \n p =", p)+
...
ggsave("figs/EJA_PVA_cum_df.png", cdf_plot)
