...
for(z in 1:length(EJA_sample)) {
  if(check_match(locus = EJA_sample[z], thresh = dist, table = ELA_sample))
    match_count <- match_count + 1
}
EJA_ELA$matches[i] <- match_count
} # Runs iterations of matches-- populates EJA_ELA$matches

nrow(EJA_ELA)
mean_EJA_ELA<- mean(EJA_ELA$matches)

##Plot and determine significance
###Observed matches
obs_loci_EJA_ELA <- data.frame(matrix(ncol = 6, nrow = 0))
colnames(obs_loci_EJA_ELA) <- colnames(EJA_raw_loci)

for(i in 1:nrow(EJA_screen_loci_select)){
if(check_match(locus = EJA_screen_loci_select$BAR[i], thresh = dist, table = ELA_screen_loci_select$BAR))
  obs_loci_EJA_ELA <- rbind(obs_loci_EJA_ELA, EJA_screen_loci_select[i,])
}

obs<-nrow(obs_loci_EJA_ELA)
obs

###Probability distribution function
ggplot(EJA_ELA, aes(x=matches)) +geom_histogram(binwidth=1) + geom_vline(aes(xintercept= mean_EJA_ELA), linetype="dashed") + geom_vline(aes(xintercept = obs), colour="green") + (labs(title= "Probability distribution function of locus matches between E. lateralis and E. jacksoni"))

###Cumulative distribution function
cdf<- data.frame(x = 1:(max(EJA_ELA$matches)+1), y = NA)
cdf$y<- cumsum(table(EJA_ELA$matches)) / nrow(EJA_ELA)

cdf_plot<- ggplot(cdf, aes(x,y)) + 
  geom_point(size=3) + 
  labs (x = "Number of matches", y = "Cumulative probability") +
  ggtitle("Cumulative probability function of matches between E. jacksoni and E. lateralis")+
